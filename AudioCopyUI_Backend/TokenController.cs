...
namespace AudioCopyUI.Backend
{
    public class TokenController
    {
        ...
        public static void Init(RouteGroupBuilder group)
        {
            ...
            if (bool.Parse(GetOrAddSettings("V1PairCompatibility", "False")))
            {
            ...
            {
                return Results.Text(Auth(token) ? "OK" : "Fail");
            });


            group.MapGet("/RequestPairID", async (HttpContext context, string udid, string name) =>
            {
                if (!DiscoveredClients.ContainsKey(udid)) return Results.UnprocessableEntity("Hasn't discovered you yet.");
            ...
            {
                if (!DiscoveredList.ContainsKey(udid)) return Results.UnprocessableEntity("Pairing is canceled or not started yet.");
                ...
                return Results.BadRequest("Selected is invalid.");
            });
